...
- [계획서 - 완료된 작업 리스트 확인](https://github.com/hyper-rookies/soeun-cdk/blob/main/%EC%9E%90%EC%97%B0%EC%96%B4%EB%A6%AC%ED%8F%AC%ED%8A%B8%EC%83%9D%EC%84%B1%EA%B3%BC%EC%A0%9C_%EC%9D%B4%EC%86%8C%EC%9D%80_%EA%B3%84%ED%9A%8D%EC%84%9C.pdf)
- [배포 링크](https://soeun-report-frontend.vercel.app/)
- [API 문서](http://43.202.135.2:8080/swagger-ui/index.html)
...
- 대시보드 데이터는 캐싱되며(TTL: 1시간), 2회 이상 접근 시 조회 성능이 향상됩니다.
...
```
> 현재는 Kakao Moment API 가 연동되어있으며, 비즈니스 토큰 발급, 광고 계정 조회, 보고서 API 호출까지 모두 정상 동작을 확인했습니다.
다만 현재는 실제 광고 집행 데이터가 없어서 빈 배열이 반환됩니다. S3에는 임시 데이터(카카오 키워드, 구글 Ads) 를 사용하고 있습니다. 실제 캠페인이 집행되면 자동으로 수집됩니다. 카카오 키워드광고는 별도 플랫폼으로 분리하는 것이 더 정확하지만, 현재는 시간 관계상 임시 데이터로 대체했고, 실제 연동은 모먼트와 동일한 방식으로 확장 가능합니다.
...
EventBridge Scheduler (매주 월 08:00 KST = UTC 일 23:00)
  ...
  → S3 저장 (reports/{report_id}.json)
  → se_conversations에 reportS3Key로 참조

Excel로 내보내기
  → EC2에서 요청이 들어올 때 Lambda 호출
  → Lambda (se-report-excel-lambda)
...
| **저장** | 저장 위치 | URL에 포함 (/shared/{token}), DB 저장 X |
...
> 현재는 EC2 단일 인스턴스로 Caffeine으로도 충분하지만, 향후 ECS 전환 가능성을 고려하여 적용했습니다.
...
#### `se_conversations` (대화방)
| 속성 | 타입 | 키 | 설명 |
|------|------|-----|------|
| `conversationId` | String | PK | 대화방 고유 ID |
| `userId` | String | GSI PK | Cognito 유저 ID |
| `title` | String | | 대화방 제목 |
| `createdAt` | Number | | 생성 시각 (Unix timestamp ms) |
| `updatedAt` | Number | GSI SK | 최종 수정 시각 (Unix timestamp ms) |
| `reportS3Key` | String | | 주간 리포트 S3 경로 (선택적) |

GSI: `userId-index` (userId) / `userId-updatedAt-index` (userId + updatedAt)

#### `se_messages` (메시지)
| 속성 | 타입 | 키 | 설명 |
|------|------|-----|------|
| `messageId` | String | PK | 메시지 고유 ID |
| `conversationId` | String | GSI PK | 소속 대화방 ID |
| `role` | String | | `user` 또는 `assistant` |
| `content` | String | | 메시지 본문 (마크다운) |
| `chartType` | String | | 차트 종류 (`bar`, `line`, `pie` 등, 선택적) |
| `structuredData` | String | | 차트/테이블 렌더링용 JSON 문자열 (선택적) |
| `createdAt` | String | GSI SK | 생성 시각 (ISO 8601) |
...
유저
 │
 ▼  userId-updatedAt-index로 최신순 조회
se_conversations
 ├── conversationId: "abc-123"
 ├── title: "매체별 전환율 비교"
 ├── reportS3Key: null                     ← 일반 채팅
 │
 └── conversationId: "report_20260309"
     reportS3Key: "reports/report_20260309.json"  ← 주간 리포트
                            │
                            └──────────────────► S3
 │
 ▼  conversationId-createdAt-index로 시간순 조회
se_messages
 ├── { role: "user",      content: "질문..." }
 ├── { role: "assistant", content: "답변...", chartType: "bar", structuredData: "[...]" }
